...
The diff pairs each local book in the series with at most one catalog entry, in two passes. A local book whose provider ID matches a catalog entry binds to it first, whatever the library order. Only then are the remaining local books matched by title, and a title match is never allowed across positions: a book the series files at position 4 cannot bind to catalog volume 9 however similar the titles are. Title matches are made strongest first, so the result does not depend on the order the library lists the books in. Two local rows for one book (a duplicate import, or an ebook and an audiobook row) compete for that one catalog entry and the loser is listed as Local only, with or without stored positions. The loser is never moved to a neighbouring entry it only resembles: a second copy of The Way of Kings does not claim The Way of Kings Prime, which stays in Missing. That is logged at DEBUG as `series diff: local book's best catalogue entry is already bound, left Local only`.
