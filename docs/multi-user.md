...
New requests notify the **Request** webhook at most once an hour for the same item from the same person, so withdrawing and asking again does not repeat the alert. Titles, authors and usernames in that webhook have mentions, Slack escapes and markdown links neutralised.
