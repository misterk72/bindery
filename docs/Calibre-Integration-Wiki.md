...
- **A 409 from the plugin counts as success.** It means Calibre already has the book; Bindery records the returned Calibre id and moves on. This is what makes re pushing idempotent. The plugin recognises a book it already holds by the `bindery` identifier Bindery stamps on every push, so this only covers books Bindery pushed before. A book Calibre already holds from somewhere else carries no such identifier and is added again.
...
4. Write integration on plugin: run **Test connection**. Note that the write integration is chosen once at startup: switching between `calibredb` and plugin mode, or changing the plugin URL, API key or push path remap, does nothing until Bindery restarts, and the skipped push is not logged. If it passes but pushes fail, the Calibre container cannot open the path Bindery sent; mount the Bindery library into it or set **Push path remap**. The log line `plugin client: server error` carries the plugin's own message. The plugin path has a separate line for the harmless case, `calibre: book already in library`, so a duplicate there is never logged as a failure.
